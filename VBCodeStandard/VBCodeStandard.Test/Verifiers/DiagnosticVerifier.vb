...
#Region " Verifier wrappers "
		...
		Private Sub VerifyDiagnostics(sources As String(), language As String, analyzer As DiagnosticAnalyzer, ParamArray expected As DiagnosticResult())
			...
			VerifyDiagnosticResults(diagnostics, analyzer, sources, expected)
		End Sub
...
#Region " Actual comparisons And verifications "
		...
		Private Shared Sub VerifyDiagnosticResults(actualResults As IEnumerable(Of Diagnostic), analyzer As DiagnosticAnalyzer, sources As String(), ParamArray expectedResults As DiagnosticResult())
...
Code:
{3}
", expectedCount, actualCount, diagnosticsOutput, String.Join(Environment.NewLine & Environment.NewLine, sources)))
			End If
			...
			For i = 0 To expectedResults.Length - 1
				...
				If Not (expected.Line = -1 AndAlso expected.Column = -1) Then
...
",
				...
				If (expected.Message IsNot Nothing AndAlso actual.GetMessage() <> expected.Message) Then
...
",
		...
		Private Shared Sub VerifyDiagnosticLocation(analyzer As DiagnosticAnalyzer, diagnostic As Diagnostic, actual As Location, expected As DiagnosticResultLocation)
			...
			Assert.IsTrue(String.Equals(actualSpan.Path, expected.Path, StringComparison.Ordinal) OrElse (actualSpan.Path IsNot Nothing AndAlso actualSpan.Path.Contains("Test0.") AndAlso expected.Path.Contains("Test.")),
				String.Format(
		...
		Private Shared Function FormatDiagnostics(analyzer As DiagnosticAnalyzer, ParamArray diagnostics As Diagnostic()) As String
			...
			For i = 0 To diagnostics.Length - 1
				...
				For Each rule In rules
					...
					If (rule IsNot Nothing AndAlso String.Equals(rule.Id, diagnostics(i).Id, StringComparison.Ordinal)) Then
						...
						If (i <> diagnostics.Length - 1) Then builder.Append(","c)
