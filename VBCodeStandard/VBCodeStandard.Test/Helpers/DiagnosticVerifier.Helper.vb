...
Namespace TestHelper
	...
	Partial Public MustInherit Class DiagnosticVerifier
		...
		Protected Shared Property AdditionalReferences As New List(Of MetadataReference)
				...
				For Each diag In diags
					...
					Else
						...
						For i = 0 To documents.Length - 1
							...
							If (tree Is diag.Location.SourceTree) Then diagnostics.Add(diag)
						Next
					End If
				Next
			Next
...
#Region " Set up compilation And documents"
		...
		Private Shared Function GetDocuments(sources As String(), language As String) As Document()
			...
			If (language <> LanguageNames.CSharp AndAlso language <> LanguageNames.VisualBasic) Then Throw New ArgumentException("Unsupported Language")

			Dim project = CreateProject(sources, language)
			Dim documents = project.Documents.ToArray()

			If (sources.Length <> documents.Length) Then Throw New SystemException("Amount of sources did not match amount of Documents created")
		...
		Private Shared Function CreateProject(sources As String(), Optional language As String = LanguageNames.CSharp) As Project
			...
			Dim fileExt As String = If(String.Equals(language, LanguageNames.CSharp, StringComparison.Ordinal), CSharpDefaultFileExt, VisualBasicDefaultExt)
			...
			AdditionalReferences.ForEach(Sub(r) solution = solution.AddMetadataReference(projectId, r))
